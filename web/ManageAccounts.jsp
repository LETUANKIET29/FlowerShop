    ...
    <body>
        <c:import url="header_loginedAdmin.jsp"></c:import>
            <form action="mainController" method="post">
                <input type="text" name="txtsearch" placeholder="Name Account">
            ...
            <% ArrayList<Account> list = (ArrayList<Account>) request.getAttribute("searchNameList"); 
           
            %>
            Total account: <%= list.size() %>
            <c:forEach var="acc" items="${requestScope.searchNameList}">
                <tr>
                    <td><c:out value="${acc.getAccID()}"></c:out></td>
                    <td><c:out value="${acc.getEmail()}"></c:out></td>
                    <td><c:out value="${acc.getFullname()}"></c:out></td>
                    <td><c:choose>
                            <c:when test="${acc.getStatus() eq 1}"><div style="color: green">active</div></c:when>
                            <c:otherwise><div style="color: red">inActive</div></c:otherwise>
                        </c:choose>
                    </td>
